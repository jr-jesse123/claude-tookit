...
def main() -> int:
    ...
    parser.add_argument("--model", required=True, choices=MODELS, help="model the task actually started with")
    parser.add_argument("--effort", choices=EFFORTS, help="effort actually used; omit for haiku")
    ...
    if args.model != "haiku" and args.effort is None:
        print(f"error: --effort is required for model {args.model!r}", file=sys.stderr)
        return 1
